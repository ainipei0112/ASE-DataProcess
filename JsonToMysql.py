...
def JsonToExcel(database, main_path, output_path, yesterday, today, wb, ws1, weekend="", output_type="both"):
    ...
    for date_directory in directories:
        if os.path.isdir(date_directory):
            ...
            # 跳過 AI_Result 中多餘資料夾
            skip_directories = ["settings", "deletion_settings NF", "deletion_settings", "Alarm"]
            if directory_name in skip_directories:
                trash.append(date_directory)
                continue
                ...
                if "OP_Checked" in data : # 確認 JSON 是否 "OP_Checked"
                    if data["AOI_Scan_Amount"] != 0 : # 確認 AOI 掃描數量不為 0
                            ...
                            try:
                                ...
                                if file_data["aoiDefectType"] in ("Blur", "blurr"):
                                    ...
                                    if(XY in OP_Crack):
                                        ...
                                        OP_Crack[XY] = 1
                                elif file_data["aoiDefectType"]=="SD Abnormal":
                                    if(XY in OP_SD_Abnormal):
                                        OP_SD_Abnormal[XY] +=1
                                    else:
                                        OP_SD_Abnormal[XY] = 1
                                    ...
                                    if(XY in OP_Exessive_Probe_Mark):
                                        ...
                                        OP_Exessive_Probe_Mark[XY] = 1
                                elif file_data["aoiDefectType"] in ("Film_Burr", "Defect Film Burr"):
                                    ...
                                    if(XY in OP_Bosch_Special_Feature):
                                        ...
                                        OP_Bosch_Special_Feature[XY] = 1
                                elif file_data["aoiDefectType"]=="Missing Expansion":
                                    if(XY in OP_Missing_Expansion):
                                        OP_Missing_Expansion[XY] +=1
                                    else:
                                        OP_Missing_Expansion[XY] = 1
                                    ...
                                    if(XY in OP_Surface_Damage):
                                        ...
                                        OP_Surface_Damage[XY] = 1
                                elif file_data["aoiDefectType"]=="Wrong size":
                                    if(XY in OP_Wrong_Size):
                                        OP_Wrong_Size[XY] +=1
                                    else:
                                        OP_Wrong_Size[XY] = 1
                                else:
                                    if(XY in OP_Others): # 'Defect','Defect Residue','Edge Die','sealring','sidesealring'
                                        OP_Others[XY] +=1
                                    else:
                                        OP_Others[XY] = 1
                            except:
                        ...
                        data_dictionary = {
                            ...
                            "OP_EA_SD_Abnormal" : len(OP_SD_Abnormal),
                            ...
                            "OP_EA_Missing_Expansion" : len(OP_Missing_Expansion),
        ...
        if output_type == "excel" or output_type == "both":
            for list in list_data2:
                ...
                data_mapping = {
                    ...
                    'AW': 'OP_EA_ChipOut',
                    'AX': 'OP_EA_Crack',
                    'AY': 'OP_EA_SD_Abnormal',
                    'AZ': 'OP_EA_Exessive_Probe_Mark',
                    'BA': 'OP_EA_Film_Burr',
                    'BB': 'OP_EA_Bosch_Special_Feature',
                    'BC': 'OP_EA_Missing_Expansion',
                    'BD': 'OP_EA_Op_Ink',
                    'BE': 'OP_EA_Pad_Damage',
                    'BF': 'OP_EA_Pad_Halo',
                    'BG': 'OP_EA_Pad_Particle',
                    'BH': 'OP_EA_Passivation_Effect',
                    'BI': 'OP_EA_Pitting_Pad',
                    'BJ': 'OP_EA_Probing_Short',
                    'BK': 'OP_EA_Residue',
                    'BL': 'OP_EA_Scratch',
...
def MailAlert():
    # Mail Server 設定
    smtp_server = '10.12.10.31'
    smtp_port = 25

    # 內容設定
    to_address = "AndyZT_Hsieh@aseglobal.com"
    subject = "2/O AOI 資料寫入錯誤"
    body = "資料寫入錯誤"

    # 建立郵件
    msg = MIMEMultipart()
    msg['From'] = "ASE-WB-2OAOI@aseglobal.com"
    msg['To'] = to_address
    msg['Subject'] = subject
