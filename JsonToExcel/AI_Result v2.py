...
# 處理 JSON 資料並寫入 Excel
def json_to_excel(database, main_path, output_path, yesterday, today, diff_day, wb, ws1, weekend=""):
    # 初始化垃圾文件和Excel文件列表
    trash = []
    excel_files = []
    list_data = []
...
# 處理 JSON 資料並寫入 Excel
wb, ws1 = resetws()
json_to_excel(database, main_path, output_path,(now + datetime.timedelta(-1)).strftime('%m%d'), now.strftime('%m%d'), 0, wb, ws1)  # 執行函數

# json_to_excel("0627","0701",-1,wb,ws1)